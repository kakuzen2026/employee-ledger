...
async function loadDispatchContracts(employeeId){
  return (await fetchDispatchContractsForEmployee(employeeId))||[];
}
// Permit raster/PDF data generated by the upload flow and existing HTTPS files.
function safeAttachmentUrl(src){
  if(typeof src!=='string')return '';
  if(/^data:(image\/(png|jpeg|gif|webp|bmp)|application\/(pdf|octet-stream|x-pdf));base64,[A-Za-z0-9+/=\r\n]+$/.test(src))return src;
...
function filePreviewHtml(src,height='100px'){
  ...
  if(!safe)return '<span role="status">添付ファイルの形式を確認してください</span>';
  const href=emp_attr(safe),size=/^\d+px$/.test(height)?height:'100px';
  return safe.startsWith('data:image/')
    ?`<a href="${href}" target="_blank" rel="noopener noreferrer"><img src="${href}" alt="添付画像" style="max-height:${size};max-width:100%;border-radius:var(--emp-radius)"></a>`
    :`<a href="${href}" target="_blank" rel="noopener noreferrer">📎 ファイルを開く</a>`;
}
